...
from pwospf import PWOSPF, PWOSPFHeader, PWOSPFHello, PWOSPFLSU, PWOSPF_PROTO
from tools import apply_mask, Int2IP
...
class InternetRouter(Thread):
    def __init__(self, switch, links=dict()):
        ...
        self.pwospf = PWOSPF(controller=self, routerID=self.switch.intfList()[2].ip, areaID=0,
                             lsuint=5)
    ...
    def handlePacket(self, pkt):
        ...
        elif IP in pkt and pkt[IP].proto == PWOSPF_PROTO:
            pwospf_pkt = PWOSPFHeader(pkt[Raw])
            if PWOSPFHello in pwospf_pkt:
                self.pwospf.receive_hello_msg(pkt)
            elif PWOSPFLSU in pwospf_pkt:
