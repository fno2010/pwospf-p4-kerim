...
class PWOSPF:
    ...
    def _trigger_hello(self, interface):
        ...
        self._send_hello_msg(src_port=interface.id, src=interface.ipv4, mask=interface.mask,
    ...
    def trigger_lsu(self):
        ...
        for interface in self._router.interfaces.values():
            for neighbor_ip in interface.neighbor_ips.values():
                self._send_lsu_msg(src_port=interface.id, src=interface.ipv4, dst=neighbor_ip,
    ...
    def receive_hello_msg(self, pkt):
        header = PWOSPFHeader(pkt[Raw])
        hello = header[PWOSPFHello]
        ...
        if pkt[CPUMetadata].srcPort in self._router.interfaces:
            ...
            if apply_mask(neighbor_ip, mask) == apply_mask(received_interface.ipv4, received_interface.mask):
